...
if(isset($_GET['id']) && $_GET['id'] > 0){
    $qry = $conn->query("SELECT * from `inventory` where inventoryID = '{$_GET['id']}' and delete_flag = 0 ");
    if($qry->num_rows > 0){
	...
	echo '<script>alert("vehicle ID is Required."); location.replace("./?page=inventory")</script>';
}
?>
<style>
...
<script>
	$(function(){
		$('#delete-vehicle').click(function(){
			_conf("Are you sure to delete this vehicle permanently?","delete_vehicle",['<?= isset($inventoryID) ? $inventoryID : '' ?>'])
		})
        $('#edit-vehicle').click(function(){
			uni_modal('<i class="fa fa-edit"></i> Add Vehicle Entry','models/manage_vehicle.php?id=<?= isset($inventoryID) ? $inventoryID : '' ?>','modal-lg')
		})
