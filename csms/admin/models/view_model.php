...
if(isset($_GET['id']) && $_GET['id'] > 0){
	$qry = $conn->query("SELECT m.*, b.manufacturerName as `manufacturer`, ct.carTypeName as `type` from `carModel` m inner join manufacturers b on m.manufacturerID = b.manufacturerID inner join carType ct on m.carTypeID = ct.carTypeID where m.modelID = '{$_GET['id']}' and m.delete_flag = 0 ");
    if($qry->num_rows > 0){
		...
		echo '<script>alert("modelID is not valid."); location.replace("./?page=models")</script>';
	}
}else{
	echo '<script>alert("modelID is Required."); location.replace("./?page=models")</script>';
}
	...
	<div class="col-lg-10 col-md-11 col-sm-12 col-xs-12">
		<div class="card rounded-0 shadow">
			<div class="card-footer py-1 text-center">
				...
				<a class="btn btn-flat btn-sm btn-navy bg-gradient-navy border" href="./?page=models/manage_model&id=<?= isset($modelID) ? $modelID : '' ?>"><i class="fa fa-edit"></i> Edit</a>
			...
			<div class="card-body">
				<div class="container-fluid">
					<div class="d-flex w-100">
						...
						<div class="col-9 mb-0 border"><?= isset($manufacturer) ? $manufacturer : '' ?></div>
					...
					<div class="d-flex w-100">
						...
						<div class="col-9 mb-0 border"><?= isset($type) ? $type : '' ?></div>
					</div>
					<div class="d-flex w-100">
						<div class="col-3 mb-0 border bg-gradient-secondary">Engine Type:</div>
						<div class="col-9 mb-0 border"><?= isset($engineType) ? $engineType : '' ?></div>
					</div>
					<div class="d-flex w-100">
						<div class="col-3 mb-0 border bg-gradient-secondary">Transmission Type:</div>
						<div class="col-9 mb-0 border"><?= isset($transmissionType) ? $transmissionType : '' ?></div>
					</div>
					<div class="d-flex w-100">
						<div class="col-3 mb-0 border bg-gradient-secondary">Description:</div>
						<div class="col-9 mb-0 border"><?= isset($description) ? htmlspecialchars_decode($description) : '' ?></div>
					</div>
				...
				<div class="container-fluid">
					<table class="table table-stripped table-bordered" id="vehicle-list">
						...
						<thead>
							...
							<th class="p-1 text-center">Vehicle Registration No.</th>
							<th class="p-1 text-center">Varient</th>
							<th class="p-1 text-center">Price</th>
							<th class="p-1 text-center">Status</th>
							<th class="p-1 text-center">Operation</th>
						</thead>
						<tbody>
							<?php if(isset($modelID)): ?>
							<?php 
							$i = 1;
							$vehicles = $conn->query("SELECT * FROM `inventory` where modelID = '{$modelID}' and delete_flag = 0 order by abs(unix_timestamp(date_created)) asc");
							while($row = $vehicles->fetch_assoc()):	
							?>
							<tr>
								<td class="p-1 align-middle text-center"><?= $i++ ?></td>
								<td class="p-1 align-middle"><?= $row['vr_number'] ?></td>
								<td class="p-1 align-middle"><?= $row['variant'] ?></td>
								...
								<td class="p-1 align-middle text-center">
									<button class="btn btn-flat btn-light bg-gradient-light btn-sm border view_vehicle" data-id="<?= $row['inventoryID'] ?>" type="button"><i class="fa fa-eye"></i> View</button>
								</td>
							</tr>
							<?php endwhile; ?>
							<?php endif; ?>
...
<script>
	$(function(){
		$('#delete_model').click(function(){
			_conf("Are you sure to delete this model permanently?","delete_model",['<?= isset($modelID) ? $modelID : '' ?>'])
		})
		$('#add_vehicle').click(function(){
			uni_modal('<i class="far fa-plus-square"></i> Add Vehicle Entry','models/manage_vehicle.php?modelID=<?= isset($modelID) ? $modelID : '' ?>','modal-lg')
		})
