...
Class Users extends DBConnection {
	...
	public function registration(){
		...
		unset($_POST['password']);
		extract($_POST);
		$main_field = ['firstname', 'lastname', 'username', 'phonenumber', 'email', 'address', 'password'];
		$data = "";
		$check = $this->conn->query("SELECT * FROM `customer` where email = '{$email}' ".($id > 0 ? " and id!='{$id}'" : "")." ")->num_rows;
		if($check > 0){
			$resp['status'] = 'failed';
		...
		if(empty($id)){
			...
			$sql = "UPDATE `customer` set {$data} where custID = '{$id}' ";
		}
		...
		if($save){
			...
			$resp['uid'] = $uid;
			if(!empty($id))
				$resp['msg'] = 'Customer Details has been updated successfully';
			else
				$resp['msg'] = 'Your Account has been created successfully';

			if(!empty($uid) && $this->settings->userdata('login_type') != 1){
				$user = $this->conn->query("SELECT * FROM `customer` where custID = '{$uid}' ");
				if($user->num_rows > 0){
		...
		if($resp['status'] == 'success' && isset($resp['msg'])){
			...
			return 2; // Return 2 for failure
		}
	}
	public function delete_customer(){
		extract($_POST);
		$qry = $this->conn->query("DELETE FROM customer where custID = $id");
		if($qry){
